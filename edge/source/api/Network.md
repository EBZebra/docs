...
SSL connection issue can occur because there is a limitation in curl for iOS platform where appropriate timeout is not used in 'select' system calls for curl SSL conection. For further details refer <a href="https://developer.apple.com/library/ios/documentation/NetworkingInternetWeb/Conceptual/NetworkingOverview/CommonPitfalls/CommonPitfalls.html">Avoiding Common Networking Mistakes</a> in iOS platform.

<pre>
It is advisable to  use IOS native Network library instead of CURL  by setting  adding  below two lines to the rhoconfig.txt file.
